...
class Collect:
    ...
    def __init__(self, symbol: str or list, interval: str or list, limit: int or list) -> None:
        ...
        self.interval = interval if type(interval) == list else [interval] * len(self.symbol)
        self.limit = limit if type(limit) == list else [limit] * len(self.symbol)
    ...
    def get_data(self, symbol, interval, limit) -> dict:
        ...
        if response.status_code == 200:
            ...
            df = pd.DataFrame(data, columns=['_'.join(x.split(' ')) for x in ['Time', 'Open', 'High', 'Low', 'Close', 'Volume', 'Close time', 'Quote asset volume', 'Number of trades', 'Taker buy base asset volume', 'Taker buy quote asset volume', 'Ignore']])
    ...
    def modify(self, df, symbol) -> None:
...
if __name__ == '__main__':
    collect = Collect(symbol=['BTCUSDT', 'ETHUSDT'], interval='1m', limit=10)
    print(collect.main())